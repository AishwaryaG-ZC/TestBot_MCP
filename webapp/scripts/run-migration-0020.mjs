...
const sql = readFileSync('./drizzle/0020_canonical_suites.sql', 'utf8')
const db = postgres(process.env.DATABASE_URL, { ssl: { rejectUnauthorized: false }, max: 1 })

try {
  await db.unsafe(sql)
  console.log('Migration 0020_canonical_suites applied successfully')

  const tables = await db`
    SELECT table_name FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name = 'project_canonical_suites'
  `
