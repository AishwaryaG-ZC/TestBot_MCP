...
class PlaywrightIntegration {
  ...
  ensureRuntimePlaywrightConfig(userConfigPath) {
    ...
    // G48: compute Playwright worker count at template-generation time so the
    // runtime config is a plain literal (no `require('os')` at config-load
    // time inside Playwright's TS evaluator). Half of available cores, capped
    // at 8 to avoid memory thrash on browser-heavy specs. HEALIX_PLAYWRIGHT_WORKERS
    // overrides for per-machine tuning.
...
// G48: workers count overridden at template-generation time to use half of
// the available CPU cores (capped at 8). HEALIX_PLAYWRIGHT_WORKERS overrides.
...
`;
  ...
  _parseProgressChunk(chunk, onProgress) {
    ...
    for (const line of lines) {
      ...
      // G26: parse project + file + suite + leaf-name out of the raw list-
      // reporter line so the dashboard can group correctly instead of dumping
      // every live test into "Other". Typical forms:
      //   "login.spec.ts \u203a should log in"
      //   "[tierA-public] \u203a login.spec.ts \u203a should log in"
      //   "[tierB-auth-admin] \u203a admin/members.spec.ts \u203a admins can manage members"
      let project = '';
      let file = '';
      let suite = '';
      let displayName = raw;
      ...
      const remainder = projectMatch ? projectMatch[2] : raw;
      if (projectMatch) project = projectMatch[1].trim();

      const segs = remainder.split(/\s*[\u203a>]\s*/);
      if (segs.length >= 2 && /\.(spec|test)\.(ts|tsx|js|jsx|mjs|cjs)$/i.test(segs[0])) {
        file = segs[0];
        displayName = segs[segs.length - 1];
        suite = segs.slice(1, -1).join(' \u203a ');
      } else {
        displayName = segs[segs.length - 1] || remainder;
        suite = segs.slice(0, -1).join(' \u203a ');
      }

      onProgress({ status, name: displayName, durationMs, file, suite, project });
    }
  }
