...
class PlaywrightIntegration {
  constructor(config = {}) {
    this.config = {
      ...
      suiteDir: config.suiteDir || 'tests/generated',
  ...
  resolveSuiteDir() {
    const raw = String(this.config.suiteDir || 'tests/generated').trim() || 'tests/generated';
    return path.isAbsolute(raw) ? raw : path.join(this.config.projectPath, raw);
  }
  ...
  ensureRuntimePlaywrightConfig(userConfigPath) {
    ...
    const testDir = this.resolveSuiteDir();
...
`;
  ...
  listGeneratedSpecFiles() {
    const testsDir = this.resolveSuiteDir();
    try {
  ...
  hasPhaseTwoTaggedTests() {
    try {
      const generatedDir = this.resolveSuiteDir();
